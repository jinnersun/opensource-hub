...
export function CategorySidebar({
  currentCat, categories, categoryId, label, description, projectCount, currentIcon, td, t
}: {
  currentCat: CatInfo | null
  categories: CatInfo[]
  categoryId: string
  label: string; description: string; projectCount: number
  currentIcon: { Icon: LucideIcon; fallback: string } | null
  td: any; t: any
}) {
  const otherCategories = categories.filter((c: CatInfo) => c.id !== categoryId).slice(0, 3)
    ...
    <aside className="w-full lg:w-72 shrink-0">
      <div className="rounded-2xl border bg-card p-6 sticky top-20">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{t('all', { label, count: projectCount })}</span>
        </div>
      </div>
      ...
      {otherCategories.length > 0 && (
        <div className="mt-6">
          ...
          <div className="space-y-2">
            {otherCategories.map((cat: CatInfo) => {
